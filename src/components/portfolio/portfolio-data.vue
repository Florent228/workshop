    ...
    <section class="relative md:py-24 py-16">
        ...
        <div class="container-fluid relative mt-8">
            <div id="grid" class="lg:columns-5 md:columns-3 sm:columns-2 columns-1 gap-2 mx-3">
                <div v-for="(item, index) in filteredData" :key="index" class="picture-item mb-2"
                    ...
                    <div class="group relative block overflow-hidden rounded-md duration-500">
                        <a @click="showLightbox(index)" class="lightbox duration-500 group-hover:scale-105 tobii-zoom" title="">
                            <img :src="item.image" class="" alt="work-image">
                        </a>
                        <div
                            class="absolute -bottom-52 group-hover:bottom-2 start-2 end-2 duration-500 bg-white dark:bg-slate-900 p-4 rounded shadow dark:shadow-gray-800">
                            <router-link to="/portfolio-detail-two"
                                class="hover:text-indigo-600 text-lg duration-500 font-medium">Iphone mockup</router-link>
            ...
            <div class="flex justify-center mt-8">
                <a href=""
                    class="py-2 px-5 inline-block font-semibold tracking-wide border align-middle duration-500 text-base text-center bg-indigo-600 hover:bg-indigo-700 border-indigo-600 hover:border-indigo-700 text-white rounded-full">See
                    More</a>
            </div>
        </div><!--end container-->
...
export default {
    ...
    methods: {
        matchCategory(category) {
            ...
            this.filteredData = this.selectedCategory ? this.datas.filter((item) => item.category === this.selectedCategory)
                : this.datas;
        },
        showLightbox(index) {
